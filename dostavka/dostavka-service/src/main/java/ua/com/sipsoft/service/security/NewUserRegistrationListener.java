...
public class NewUserRegistrationListener implements
		ApplicationListener<OnRegistrationCompleteEvent>, VerificationTokenGenerator {
	...
	private void confirmRegistration(OnRegistrationCompleteEvent event) {
		log.info("Create Verification Token and Send confirmation email.");
		SimpleMailMessage email = new SimpleMailMessage();
		User user = event.getUser();
		String token = makeVerificationToken();

		VerificationToken vToken = userService.createVerificationToken(user, token, VerificationTokenType.REGNEWUSER);
		if (vToken == null) {
		...
		String confirmationUrl = AppURL.APP_DOMAIN + servletContext.getContextPath() + "/" + AppURL.REGISTRATION_CONFIRM + "?token="
				+ token;
		...
		AppNotificator.notify(i18n.getTranslation(LoginMsg.REG_CONFIRM_MSG_SENDED, event.getLocale()));
	}
}